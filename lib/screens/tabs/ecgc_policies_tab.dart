...
class EcgcPoliciesTab extends StatefulWidget {
  final String category;
  final String title;
  final String? initialSection;

  const EcgcPoliciesTab({
    super.key,
    this.category = 'ECGC',
    this.title = 'ECGC Insurance',
    this.initialSection,
  });

  @override
  State<EcgcPoliciesTab> createState() => _EcgcPoliciesTabState();
}

class _EcgcPoliciesTabState extends State<EcgcPoliciesTab> {
  ...
  static const _policySections = [
    ...
    'Factoring Services',
    'Letter of Credit Confirmation (LCC)',
    'Other',
  ];
          ...
          body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, snap) {
              ...
              return GridView.builder(
                ...
                itemBuilder: (context, i) => _EcgcPolicyGridCard(
                  doc: docs[i],
                  serialNumber: i + 1,
  ...
  Future<void> _openPolicyDialog(
    ...
    final rawGroups = existing?['ecgcCommissions'];
...
class _EcgcPolicyGridCard extends StatefulWidget {
  final QueryDocumentSnapshot<Map<String, dynamic>> doc;
  final int serialNumber;
  final VoidCallback onTap;

  const _EcgcPolicyGridCard({
    required this.doc,
  ...
  @override
  State<_EcgcPolicyGridCard> createState() => _EcgcPolicyGridCardState();
}

class _EcgcPolicyGridCardState extends State<_EcgcPolicyGridCard> {
  bool _isHovered = false;

  static int _slabCount(Map<String, dynamic> data) {
    final groups = data['ecgcCommissions'];
  ...
  Widget build(BuildContext context) {
    ...
    final planName = (data['planName'] ?? 'ECGC Policy').toString();
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: _EcgcPoliciesTabState._surface,
            borderRadius: BorderRadius.circular(16),
            border: Border.all(
              color: _isHovered
                  ? _EcgcPoliciesTabState._accent
                  : _EcgcPoliciesTabState._border,
              width: _isHovered ? 1.5 : 1.0,
            ),
            boxShadow: [
              if (_isHovered)
                BoxShadow(
                  color: _EcgcPoliciesTabState._accent.withValues(alpha: 0.08),
                  blurRadius: 16,
            ...
            children: [
              Positioned(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _EcgcPoliciesTabState._bg,
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: _EcgcPoliciesTabState._border),
                  ),
                  child: Text(
                    '#${widget.serialNumber}',
                    style: const TextStyle(
                      color: _EcgcPoliciesTabState._textMuted,
                      fontSize: 10,
                      fontWeight: FontWeight.w700,
                    ),
                  ),
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          ...
                          overflow: TextOverflow.ellipsis,
                          style: const TextStyle(
                            color: _EcgcPoliciesTabState._textMain,
                            fontSize: 13,
                            fontWeight: FontWeight.w700,
                            height: 1.25,
                          ),
...
class _GeneralPolicyDetailView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: _EcgcPoliciesTabState._bg,
      body: SafeArea(
        child: Column(
          children: [
            Container(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          (data['planName'] ?? 'ECGC Policy').toString(),
                          style: const TextStyle(
                            color: _EcgcPoliciesTabState._textMain,
                            fontSize: 18,
                  ...
                  TextButton.icon(
                    ...
                    style: TextButton.styleFrom(
                      foregroundColor: _EcgcPoliciesTabState._red,
                    ),
                  ),
                ],
              ),
            ),
            const Divider(height: 1, color: _EcgcPoliciesTabState._border),
            Expanded(
              child: ListView(
                padding: const EdgeInsets.all(20),
                children: [
  ...
  Widget _slabGroup(dynamic group) {
    ...
    if (slabs is! List || slabs.isEmpty) {
      return const Text(
        'No slabs found.',
        style: TextStyle(color: _EcgcPoliciesTabState._textMuted),
      );
    }
    return SingleChildScrollView(
      ...
      child: DataTable(
        headingRowColor: WidgetStatePropertyAll(
          _EcgcPoliciesTabState._primary.withValues(alpha: 0.06),
        ),
        columns: const [
  ...
  Future<void> _delete(BuildContext context) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: _EcgcPoliciesTabState._red,
              foregroundColor: Colors.white,
            ),
    ...
    final data = doc.data();
    final deletedPlanName = (data['planName'] ?? 'ECGC Policy').toString();
    await FirebaseFirestore.instance
        .collection('ecgc_policies')
        .doc(doc.id)
        .delete();
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          ...
          backgroundColor: _EcgcPoliciesTabState._primary,
        ),
      );
      onDeleted();
    }
      ...
      child: Row(
        children: [
          Container(
            ...
            height: 38,
            decoration: BoxDecoration(
              color: _EcgcPoliciesTabState._accent.withValues(alpha: 0.1),
              borderRadius: BorderRadius.circular(8),
            ),
            child: Icon(icon, color: _EcgcPoliciesTabState._accent, size: 20),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
