...
class AgriculturePoliciesTab extends StatefulWidget {
  final String category;
  final String title;
  final String? initialSection;

  const AgriculturePoliciesTab({
    super.key,
    this.category = 'Agriculture',
    this.title = 'Agriculture Insurance',
    this.initialSection,
  });

  @override
  State<AgriculturePoliciesTab> createState() => _AgriculturePoliciesTabState();
}

class _AgriculturePoliciesTabState extends State<AgriculturePoliciesTab> {
          ...
          body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, snap) {
              ...
              var docs = (snap.data?.docs ?? [])
                  .where(
                    (d) =>
                        (d.data()['category'] ?? widget.category).toString() ==
                        widget.category,
                  )
                  .toList();
              ...
              return GridView.builder(
                ...
                itemBuilder: (context, i) => _AgriculturePolicyGridCard(
                  doc: docs[i],
                  serialNumber: i + 1,
  ...
  Future<void> _openPolicyDialog(
    ...
    final rawGroups = existing?['agriculturalCommissions'];
...
class _AgriculturePolicyGridCard extends StatefulWidget {
  final QueryDocumentSnapshot<Map<String, dynamic>> doc;
  final int serialNumber;
  final VoidCallback onTap;

  const _AgriculturePolicyGridCard({
    required this.doc,
  ...
  @override
  State<_AgriculturePolicyGridCard> createState() =>
      _AgriculturePolicyGridCardState();
}

class _AgriculturePolicyGridCardState
    extends State<_AgriculturePolicyGridCard> {
  bool _isHovered = false;

  static int _slabCount(Map<String, dynamic> data) {
    final groups = data['agriculturalCommissions'];
  ...
  Widget build(BuildContext context) {
    ...
    final planName = (data['planName'] ?? 'Agriculture Policy').toString();
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: _AgriculturePoliciesTabState._surface,
            borderRadius: BorderRadius.circular(16),
            border: Border.all(
              color: _isHovered
                  ? _AgriculturePoliciesTabState._accent
                  : _AgriculturePoliciesTabState._border,
              width: _isHovered ? 1.5 : 1.0,
            ),
            boxShadow: [
              if (_isHovered)
                BoxShadow(
                  color: _AgriculturePoliciesTabState._accent.withValues(
                    alpha: 0.08,
                  ),
                  blurRadius: 16,
            ...
            children: [
              Positioned(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _AgriculturePoliciesTabState._bg,
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(
                      color: _AgriculturePoliciesTabState._border,
                    ),
                  ),
                  child: Text(
                    '#${widget.serialNumber}',
                    style: const TextStyle(
                      color: _AgriculturePoliciesTabState._textMuted,
                      fontSize: 10,
                      fontWeight: FontWeight.w700,
                    ),
                  ),
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          ...
                          overflow: TextOverflow.ellipsis,
                          style: const TextStyle(
                            color: _AgriculturePoliciesTabState._textMain,
                            fontSize: 13,
                            fontWeight: FontWeight.w700,
                            height: 1.25,
                          ),
...
class _GeneralPolicyDetailView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: _AgriculturePoliciesTabState._bg,
      body: SafeArea(
        child: Column(
          children: [
            Container(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          (data['planName'] ?? 'Agriculture Policy').toString(),
                          style: const TextStyle(
                            color: _AgriculturePoliciesTabState._textMain,
                            fontSize: 18,
                  ...
                  TextButton.icon(
                    ...
                    style: TextButton.styleFrom(
                      foregroundColor: _AgriculturePoliciesTabState._red,
                    ),
                  ),
                ],
              ),
  ...
  Widget _slabGroup(dynamic group) {
    ...
    if (slabs is! List || slabs.isEmpty) {
      return const Text(
        'No slabs found.',
        style: TextStyle(color: _AgriculturePoliciesTabState._textMuted),
      );
    }
    return SingleChildScrollView(
      ...
      child: DataTable(
        headingRowColor: WidgetStatePropertyAll(
          _AgriculturePoliciesTabState._primary.withValues(alpha: 0.06),
        ),
        columns: const [
  ...
  Future<void> _delete(BuildContext context) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: _AgriculturePoliciesTabState._red,
              foregroundColor: Colors.white,
            ),
    ...
    final data = doc.data();
    final deletedPlanName = (data['planName'] ?? 'Agriculture Policy')
        .toString();
    await FirebaseFirestore.instance
        .collection('agricultural_policies')
        .doc(doc.id)
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          ...
          backgroundColor: _AgriculturePoliciesTabState._primary,
        ),
      );
      onDeleted();
    }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: _AgriculturePoliciesTabState._surface,
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: _AgriculturePoliciesTabState._border),
      ),
      child: Row(
        children: [
          Container(
            ...
            child: Icon(
              icon,
              color: _AgriculturePoliciesTabState._accent,
              size: 20,
            ),
          ),
          const SizedBox(width: 12),
