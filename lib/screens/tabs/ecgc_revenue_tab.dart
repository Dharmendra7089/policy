...
class EcgcRevenueTab extends StatefulWidget {
  final String category;
  final String title;

  const EcgcRevenueTab({
    super.key,
    this.category = 'ECGC',
    this.title = 'ECGC Revenue',
  });

  @override
  State<EcgcRevenueTab> createState() => _EcgcRevenueTabState();
}

class _EcgcRevenueTabState extends State<EcgcRevenueTab> {
  ...
  double _effectivePct(Map<String, dynamic> row, Map<String, dynamic> policy) {
    ...
    final slab = row['selectedEcgcSlab'];
    if (slab is Map) {
      ...
      builder: (context, snap) {
        ...
        return ListView(
          ...
          children: [
            ...
            const Text(
              'Choose a ECGC policy, then select start and end dates.',
              style: TextStyle(color: _textMuted, fontSize: 13),
            ),
            const SizedBox(height: 16),
            DropdownButtonFormField<String>(
              initialValue: _selectedPolicyId,
              hint: const Text('Select ECGC policy'),
              isExpanded: true,
...
class _Card extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: _EcgcRevenueTabState._surface,
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: _EcgcRevenueTabState._border),
      ),
      child: Row(
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  value,
                  style: const TextStyle(
                    color: _EcgcRevenueTabState._textMain,
                    fontSize: 18,
                    fontWeight: FontWeight.w900,
                  ),
                ),
                Text(
                  title,
                  style: const TextStyle(
                    color: _EcgcRevenueTabState._textMuted,
                    fontSize: 12,
                    fontWeight: FontWeight.w600,
                  ),
                ),
