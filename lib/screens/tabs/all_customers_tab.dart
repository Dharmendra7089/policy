...
class _AllCustomersTabState extends State<AllCustomersTab> {
  ...
  Future<void> _openLinkPolicyDialog(
    ...
    final allPolicies = uniquePolicies.values.toList()
      ..removeWhere(
        (policy) =>
            (policy['category'] ?? 'Health').toString().toLowerCase() !=
    ...
    String selectedCommissionSlabLabel = '';
            ...
            void recomputeCommission() {
              ...
              if (category == 'health') {
                  ...
                  if (activeGroup != null) {
                    ...
                    if (slabs is List) {
                      ...
                      final compareValue = type == 'sumInsured'
                          ...
                          // Read the controller directly. The `premium`
                          // variable above belongs to the current builder
                          // frame and is one keystroke behind inside onChanged.
                          : parseNum(premiumCtrl.text);
                      for (final s in slabs) {
                        ...
                        if (compareValue >= from && compareValue <= to) {
                          setS(() {
                            setCommissionPercentValue(pct);
                            selectedCommissionSlabLabel =
                                s['label']?.toString() ?? 'Health policy slab';
                          });
                          return;
                        }
                      }
                      // New Health business must never fall through to the
                      // renewal percentage. Before premium/sum insured is
                      // entered, show and save the first configured base slab.
                      if (baseSlab != null) {
                        final basePct =
                            double.tryParse(
                              baseSlab['percent']?.toString() ?? '0',
                            ) ??
                            0;
                        setS(() {
                          setCommissionPercentValue(basePct);
                          selectedCommissionSlabLabel =
                              baseSlab!['label']?.toString() ??
                              'Health base commission';
                        });
                        return;
                      }
                    }
                ...
                setS(() {
                  ...
                  selectedCommissionSlabLabel =
                      'No Health commission slab configured';
                });
                return;
              }
              setS(() {
                ...
                selectedCommissionSlabLabel = 'Renewal commission';
              });
            }
            ...
            Future<void> saveLink() async {
              ...
              try {
                ...
                final policyFields = {
                  ...
                  'selectedSlabPercent': commissionPercent,
                  'selectedSlabLabel': selectedCommissionSlabLabel,
                  'commissionRateSource': 'policySlab',
...
class _CustomerDetailView extends StatelessWidget {
  ...
  Widget _buildContent(
    ...
    final baseCommissionPct =
        policyData['baseCommissionPercent']?.toString() ?? '';
                  ...
                  children: [
                    ...
                    if (linkedPolicy != null) ...[
                      ...
                      _excelCard('Commission', [
                        if (baseCommissionPct.isNotEmpty)
                          _xlRow('Base Commission %', '$baseCommissionPct%'),
                        _xlRow(
                          baseCommissionPct.isNotEmpty
