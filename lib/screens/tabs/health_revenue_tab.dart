...
class HealthRevenueTab extends StatefulWidget {
  const HealthRevenueTab({super.key});

  @override
  State<HealthRevenueTab> createState() => _HealthRevenueTabState();
}

class _HealthRevenueTabState extends State<HealthRevenueTab> {
  ...
  Map<String, dynamic>? _activeHealthGroup(Map<String, dynamic> policy) {
    final hc = policy['healthCommissions'];
    if (hc is! List || hc.isEmpty) return null;
    ...
    if (hc.last is Map) return Map<String, dynamic>.from(hc.last as Map);
    return null;
  }

  double _slabPercentForSumInsured(
    Map<String, dynamic> policy,
    double sumInsured,
  ) {
    final group = _activeHealthGroup(policy);
    if (group == null) {
      return _num(
    ...
    for (final s in slabs) {
      ...
      final from = _num(slab['fromAmount']);
      final toRaw = slab['toAmount'];
      final to = toRaw == null ? double.infinity : _num(toRaw);
      if (sumInsured >= from && sumInsured <= to && from >= bestFrom) {
        bestFrom = from;
        best = slab;
      }
    }
    if (best != null) return _num(best['percent']);
    return _num(
      policy['renewalCommission'] ?? policy['renewalCommissionPercent'],
    );
  }

  /// For a not-settled row: use manualSlabPercent if set, else slab lookup.
  double _effectivePct(Map<String, dynamic> row, Map<String, dynamic> policy) {
    if (policy['commissionRules'] is List) {
      return CommissionEngine.calculate(policy, row).percent;
    }
    final manual = _num(row['manualSlabPercent']);
    if (manual > 0) return manual;
    final sumInsured = _num(row['sumInsured'] ?? row['policySumInsured']);
    return _slabPercentForSumInsured(policy, sumInsured);
  }
  ...
  double _commissionAmountForRow(
    ...
    final calculationMode = (row['commissionCalculationMode'] ?? '').toString();
    if (row['commissionAmountLocked'] == true ||
        calculationMode == 'workbookAmount') {
      return _num(row['commissionAmountOverride'] ?? row['commissionAmount']);
    }
  ...
  Future<void> _markSelectedSettled(
    ...
    try {
      ...
      for (final id in _selectedRows) {
        ...
        final commission = _commissionAmountForRow(data, policy);
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snap) {
        ...
        if (policies.isEmpty) {
          return const Center(
            child: Text(
              'No active Health policies found.',
        ...
        return ListView(
          ...
          children: [
            ...
            const Text(
              'Choose a Health policy, then select start and end dates.',
              style: TextStyle(color: _textMuted, fontSize: 13),
            ),
            const SizedBox(height: 16),
            DropdownButtonFormField<String>(
              initialValue: _selectedPolicyId,
              hint: const Text('Select Health Policy'),
              isExpanded: true,
  ...
  Widget _selectedPolicyView() {
    ...
    final healthCommissions = policy['healthCommissions'];
    final slabsGroup =
        (healthCommissions is List && healthCommissions.isNotEmpty)
        ? healthCommissions.first
        : null;
    final slabs = (slabsGroup is Map) ? slabsGroup['slabs'] : null;
    final slabType = (slabsGroup is Map)
        ? slabsGroup['type']?.toString() ?? 'sumInsured'
        : 'sumInsured';
    final slabStart = (slabsGroup is Map)
        ? _fmtDate(slabsGroup['startDate'])
        : '-';
    final slabEnd = (slabsGroup is Map) ? _fmtDate(slabsGroup['endDate']) : '-';
            ...
            _Panel(
              child: Column(
                ...
                children: [
                  ...
                  _detailRow('Slab Type', slabType),
                  _detailRow('Slab Start', slabStart),
                  _detailRow('Slab End', slabEnd),
                  _detailRow(
                    'Selected From',
                  ...
                  if (slabs is! List || slabs.isEmpty)
                    ...
                    SingleChildScrollView(
                      ...
                      child: DataTable(
                        ...
                        rows: slabs.map<DataRow>((s) {
                          final m = s is Map ? s : <String, dynamic>{};
                          return DataRow(
                            cells: [
                              DataCell(Text((m['label'] ?? '').toString())),
                              DataCell(
                                Text(_num(m['fromAmount']).toStringAsFixed(0)),
                              ),
                              DataCell(
                                Text(
                                  m['toAmount'] == null
                                      ? 'Above'
                                      : _num(m['toAmount']).toStringAsFixed(0),
                                ),
                              ),
                      ...
                      child: DataTable(
                        ...
                        rows: docs.map((d) {
                          ...
                          final commission = settled
                              ...
                              : _commissionAmountForRow(data, policy);

                          final hasManual =
                              (_num(data['manualSlabPercent']) > 0 ||
                                  data['commissionAmountLocked'] == true) &&
                              !settled;
...
class _Card extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: _HealthRevenueTabState._surface,
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: _HealthRevenueTabState._border),
      ),
      child: Row(
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  value,
                  style: const TextStyle(
                    color: _HealthRevenueTabState._textMain,
                    fontSize: 18,
                    fontWeight: FontWeight.w900,
                  ),
                ),
                Text(
                  title,
                  style: const TextStyle(
                    color: _HealthRevenueTabState._textMuted,
                    fontSize: 12,
                    fontWeight: FontWeight.w600,
                  ),
                ),
