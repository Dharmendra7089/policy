...
class GeneralRevenueTab extends StatefulWidget {
  final String category;
  final String title;

  const GeneralRevenueTab({
    super.key,
    this.category = 'General',
    this.title = 'General Revenue',
  });

  @override
  State<GeneralRevenueTab> createState() => _GeneralRevenueTabState();
}

class _GeneralRevenueTabState extends State<GeneralRevenueTab> {
  ...
  double _effectivePct(Map<String, dynamic> row, Map<String, dynamic> policy) {
    ...
    final slab = row['selectedGeneralSlab'];
    if (slab is Map) {
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snap) {
        ...
        if (policies.isEmpty) {
          return const Center(
            child: Text(
              'No active General policies found.',
        ...
        return ListView(
          ...
          children: [
            ...
            const Text(
              'Choose a General policy, then select start and end dates.',
              style: TextStyle(color: _textMuted, fontSize: 13),
            ),
            const SizedBox(height: 16),
            DropdownButtonFormField<String>(
              initialValue: _selectedPolicyId,
              hint: const Text('Select General policy'),
              isExpanded: true,
...
class _Card extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: _GeneralRevenueTabState._surface,
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: _GeneralRevenueTabState._border),
      ),
      child: Row(
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  value,
                  style: const TextStyle(
                    color: _GeneralRevenueTabState._textMain,
                    fontSize: 18,
                    fontWeight: FontWeight.w900,
                  ),
                ),
                Text(
                  title,
                  style: const TextStyle(
                    color: _GeneralRevenueTabState._textMuted,
                    fontSize: 12,
                    fontWeight: FontWeight.w600,
                  ),
                ),
